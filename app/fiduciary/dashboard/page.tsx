...
const Card = ({ children, className }: any) => (
  <div
    className={`bg-black text-white border border-gray-700 rounded-xl p-6 shadow-md ${
      className || ""
    }`}
  >
    {children}
...
const Button = ({ children, onClick, disabled, className }: any) => (
  <button
    onClick={onClick}
    disabled={disabled}
    className={`px-5 py-2 rounded-lg font-medium border border-gray-600 hover:bg-green-700 hover:text-white transition ${
      disabled ? "bg-gray-700 cursor-not-allowed" : "bg-green-600"
    } ${className}`}
  >
    {children}
  </button>
...
export default function TokenizationDashboardMock() {
  ...
  const createProject = () => {
    ...
    setTimeout(() => {
      ...
      alert(
        `✅ Proyecto "${projectName}" creado! Estado: PENDING. ID: MOCK${newId}`
      );
      setIpfs("");
      setProjectName("");
      setGoal("");
  ...
  const deployFideiWallet = async () => {
    try {
      ...
      const tx = await factory.deployCustody(
        selectedCampaign,
        fiduciario,
        auditor,
        tokenAddress
  ...
  const deployModules = async () => {
    try {
      ...
      const factory = new ethers.Contract(FACTORY_ADDRESS, FACTORY_ABI, signer);
      const fiduciario = await signer.getAddress();
      const tx = await factory.deployModules(
        selectedCampaign,
        fiduciario,
        "FIDEI",
        "FID"
  ...
  const submitMilestone = () => {
    ...
    const releaseRealMilestone = async () => {
      try {
        ...
        const tx = await custody.releaseMilestone(
          currentWallet.campaignId,
          0,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <Button
                onClick={deployFideiWallet}
                disabled={loading}
                className="w-full mb-2"
              >
                {loading ? "Procesando..." : "Crear FideiWallet"}
              </Button>
              <Button
                onClick={deployModules}
                disabled={loading}
                className="w-full"
              >
                {loading ? "Procesando..." : "Desplegar Módulos"}
              </Button>
            </CardContent>
          </Card>
        </div>
          ...
          <ul className="space-y-3">
            {campaigns.map((c) => (
              ...
              >
                ...
                {c.balance && <p>💰 Balance Custody: {c.balance} mUSDT</p>}
              </li>
            ))}
          </ul>
        </div>
