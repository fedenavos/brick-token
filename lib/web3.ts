...
export const client = createThirdwebClient({
  ...
  secretKey: process.env.NEXT_PUBLIC_THIRDWED_SECRET_KEY || "your-secret-key",
});
    ...
    if (USE_MOCKS) {
      ...
      const mockProject = {
        ...
        descripcion: {
          id: "desc-001",
          descripcion:
            "Complejo residencial de 120 unidades en zona premium de Palermo",
...
export function useSubmitEvidence() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const txHash = await web3Service.submitEvidence(
        projectId,
        milestoneId,
        evidenceUri,
        wallet
...
export function useMilestoneApproval() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (action === "approve") {
        txHash = await web3Service.approveMilestone(
          projectId,
          milestoneId,
          wallet
        );
      } else {
        txHash = await web3Service.rejectMilestone(
          projectId,
          milestoneId,
          reason || "",
          wallet
...
export function useNetworkValidation() {
  ...
  return {
    targetChainId,
    isCorrectNetwork: (currentChainId?: number) =>
